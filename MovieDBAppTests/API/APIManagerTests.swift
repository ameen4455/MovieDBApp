...
final class APIManagerTests: XCTestCase {
    ...
    func testRequest_returnsDecodedData_forValidResponse() async throws {
        ...
        let httpResponse = HTTPURLResponse(url: URL(string: urlString)!, statusCode: 200, httpVersion: nil, headerFields: nil)!
    ...
    func testRequest_throwsError_forNon2xxStatusCode() async {
        ...
        let httpResponse = HTTPURLResponse(url: URL(string: urlString)!, statusCode: 404, httpVersion: nil, headerFields: nil)!
        ...
        do {
            ...
            XCTFail("Expected NetworkError but received a different error: \(error.localizedDescription)")
        }
    }
   ...
   func testRequest_throwsDecodingError_forInvalidJSON() async {
       ...
       let invalidJsonData = "{\"id\":\"not_an_int\",\"name\":\"Bad Item\"}".data(using: .utf8)!
   ...
   func testRequest_throwsNetworkFailureError_forNoInternetConnection() async {
       ...
       let networkConnectionError = URLError(.notConnectedToInternet) // Or .cannotConnectToHost, .timedOut
